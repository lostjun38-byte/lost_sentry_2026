...
namespace velocity_smoother_ext
{

VelocitySmootherExt::VelocitySmootherExt(const rclcpp::NodeOptions & options)
: LifecycleNode("velocity_smoother_ext", "", options),
  last_command_time_{0, 0, get_clock()->get_clock_type()}
{
}

VelocitySmootherExt::~VelocitySmootherExt()
{
  if (timer_) {
    timer_->cancel();
    timer_.reset();
...
VelocitySmootherExt::on_configure(const rclcpp_lifecycle::State &)
{
  ...
  declare_parameter_if_not_declared(node, "odom_topic", rclcpp::ParameterValue("odometry"));
  ...
  cmd_sub_ = create_subscription<geometry_msgs::msg::Twist>(
    ...
    std::bind(&VelocitySmootherExt::inputCommandCallback, this, std::placeholders::_1));

  speed_up_sub_ = create_subscription<std_msgs::msg::Float32>(
    "speed_up", rclcpp::QoS(1),
    std::bind(&VelocitySmootherExt::speedUpCallback, this, std::placeholders::_1));

  return nav2_util::CallbackReturn::SUCCESS;
}

nav2_util::CallbackReturn
VelocitySmootherExt::on_activate(const rclcpp_lifecycle::State &)
{
  ...
  timer_ = create_wall_timer(
    ...
    std::bind(&VelocitySmootherExt::smootherTimer, this));

  dyn_params_handler_ = this->add_on_set_parameters_callback(
    std::bind(&VelocitySmootherExt::dynamicParametersCallback, this, _1));
...
nav2_util::CallbackReturn
VelocitySmootherExt::on_deactivate(const rclcpp_lifecycle::State &)
{
...
nav2_util::CallbackReturn
VelocitySmootherExt::on_cleanup(const rclcpp_lifecycle::State &)
{
...
nav2_util::CallbackReturn
VelocitySmootherExt::on_shutdown(const rclcpp_lifecycle::State &)
{
  RCLCPP_INFO(get_logger(), "Shutting down");
  return nav2_util::CallbackReturn::SUCCESS;
}

void VelocitySmootherExt::inputCommandCallback(const geometry_msgs::msg::Twist::SharedPtr msg)
{
...
void VelocitySmootherExt::speedUpCallback(const std_msgs::msg::Float32::SharedPtr msg)
{
  speed_up_rate_ = msg->data;
}

double VelocitySmootherExt::findEtaConstraint(
...
double VelocitySmootherExt::applyConstraints(
...
void VelocitySmootherExt::smootherTimer()
{
  ...
  cmd_vel->linear.x *= speed_up_rate_;
  cmd_vel->linear.y *= speed_up_rate_;
  cmd_vel->angular.z *= speed_up_rate_;

  smoothed_cmd_pub_->publish(std::move(cmd_vel));
}

rcl_interfaces::msg::SetParametersResult
VelocitySmootherExt::dynamicParametersCallback(std::vector<rclcpp::Parameter> parameters)
{
...
RCLCPP_COMPONENTS_REGISTER_NODE(velocity_smoother_ext::VelocitySmootherExt)
