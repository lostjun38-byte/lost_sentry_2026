...
namespace small_point_lio {
    ...
    SmallPointLioNode::SmallPointLioNode(const rclcpp::NodeOptions &options)
        ...
        std::string lidar_frame_fake = declare_parameter<std::string>("lidar_frame_fake");
        ...
        map_save_trigger = create_service<std_srvs::srv::Trigger>(
                "map_save",
                [this, save_pcd, lidar_frame_fake](const std_srvs::srv::Trigger::Request::SharedPtr req, std_srvs::srv::Trigger::Response::SharedPtr res) {
                    if (!save_pcd) {
                        res->success = false;
                    ...
                    *pointcloud_to_save = pointcloud_mapping->get_points();
                    std::thread([pointcloud_to_save, lidar_frame_fake]() {
                        io::pcd::write_pcd(ROOT_DIR + "/pcd/scan.pcd", *pointcloud_to_save);
                        RCLCPP_INFO(rclcpp::get_logger("small_point_lio"), "save pcd success");
                    }).detach();
                });
        small_point_lio->set_odometry_callback([this, lidar_frame_fake](const common::Odometry &odometry) {
            ...
            transform_stamped.header.frame_id = "odom_fake";
            transform_stamped.child_frame_id = "base_link_fake";
            geometry_msgs::msg::TransformStamped base_link_fake_to_lidar_frame_fake_transform;
            try {
                base_link_fake_to_lidar_frame_fake_transform = tf_buffer->lookupTransform(lidar_frame_fake, "base_link_fake", time_msg);
            } catch (tf2::TransformException &ex) {
                RCLCPP_ERROR(rclcpp::get_logger("small_point_lio"), "Failed to lookup transform from base_link_fake to %s: %s", lidar_frame_fake.c_str(), ex.what());
                return;
            }
            tf2::Transform tf_lidar_odom_to_lidar_frame_fake;
            tf_lidar_odom_to_lidar_frame_fake.setOrigin(tf2::Vector3(odometry.position.x(), odometry.position.y(), odometry.position.z()));
            tf_lidar_odom_to_lidar_frame_fake.setRotation(tf2::Quaternion(odometry.orientation.x(), odometry.orientation.y(), odometry.orientation.z(), odometry.orientation.w()));
            tf2::Transform tf_base_link_fake_to_lidar_frame_fake;
            tf2::fromMsg(base_link_fake_to_lidar_frame_fake_transform.transform, tf_base_link_fake_to_lidar_frame_fake);
            tf2::Transform tf_odom_to_base_link_fake = tf_base_link_fake_to_lidar_frame_fake.inverse() * tf_lidar_odom_to_lidar_frame_fake * tf_base_link_fake_to_lidar_frame_fake;
            transform_stamped.transform = tf2::toMsg(tf_odom_to_base_link_fake);

            nav_msgs::msg::Odometry odometry_msg;
            odometry_msg.header.stamp = time_msg;
            odometry_msg.header.frame_id = "odom_fake";
            odometry_msg.child_frame_id = "base_link_fake";
        ...
        small_point_lio->set_pointcloud_callback([this, save_pcd, lidar_frame_fake](const std::vector<Eigen::Vector3f> &pointcloud) {
            if (pointcloud_publisher->get_subscription_count() > 0) {
                ...
                geometry_msgs::msg::TransformStamped lidar_frame_fake_to_base_link_fake_transform;
                try {
                    lidar_frame_fake_to_base_link_fake_transform = tf_buffer->lookupTransform("base_link_fake", lidar_frame_fake, time_msg);
                } catch (tf2::TransformException &ex) {
                    RCLCPP_ERROR(rclcpp::get_logger("small_point_lio"), "Failed to lookup transform from %s to base_link_fake: %s", lidar_frame_fake.c_str(), ex.what());
                    return;
                }
                Eigen::Vector3f lidar_frame_fake_to_base_link_fake_T;
                lidar_frame_fake_to_base_link_fake_T << static_cast<float>(lidar_frame_fake_to_base_link_fake_transform.transform.translation.x),
                        static_cast<float>(lidar_frame_fake_to_base_link_fake_transform.transform.translation.y),
                        static_cast<float>(lidar_frame_fake_to_base_link_fake_transform.transform.translation.z);
                Eigen::Matrix3f lidar_frame_fake_to_base_link_fake_R =
                        Eigen::Quaternionf(
                                static_cast<float>(lidar_frame_fake_to_base_link_fake_transform.transform.rotation.w),
                                static_cast<float>(lidar_frame_fake_to_base_link_fake_transform.transform.rotation.x),
                                static_cast<float>(lidar_frame_fake_to_base_link_fake_transform.transform.rotation.y),
                                static_cast<float>(lidar_frame_fake_to_base_link_fake_transform.transform.rotation.z))
                                .toRotationMatrix();
                sensor_msgs::msg::PointCloud2 msg;
                msg.header.stamp = time_msg;
                msg.header.frame_id = "odom_fake";
                ...
                for (const auto &point: pointcloud) {
                    transformed_point = lidar_frame_fake_to_base_link_fake_R * point + lidar_frame_fake_to_base_link_fake_T;
